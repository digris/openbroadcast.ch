...
# TODO: this is code for legacy (v1) API.
...
REQUEST_HEADERS = {"User-Agent": "openbroadcast.ch"}
...
def get_remote_votes(obj_ct, obj_pk):
    url = "{base_url}v1/rating/vote/{obj_ct}/{obj_pk}/".format(
        base_url=API_BASE_URL, obj_ct=obj_ct, obj_pk=obj_pk
    )
    ...
    headers.update(
        {
            "Authorization": "ApiKey {username}:{api_key}".format(
                username=API_BASE_AUTH["username"], api_key=API_BASE_AUTH["api_key"]
            )
        }
    )
