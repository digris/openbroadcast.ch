...
def fetch_from_api(range_start=0, range_end=0, channel=None):
    ...
    url = (
        API_BASE_URL
        + "v1/abcast/channel/%s/schedule/?range_start=%s&range_end=%s"
        % (CHANNEL_ID, range_start, range_end)
    )
    headers = {
    ...
    qs = ScheduledItem.objects.filter(
        ...
        time_start__lte=now + datetime.timedelta(seconds=range_end),
    ).order_by("-time_start")
...
def cleanup_old_entries(max_age):
    ...
    log.debug("%s scheduled items to delete" % old_entries.count())
    old_entries.delete()
