...
var BPlayerApp = function () {
     ...
     */
    this.mode = 'local'; // available: 'inline', 'popup', 'local'
    this.popup = false;
    this.local = false;
    ...
    this.get_remote_window = function () {
        ...
        if (win == undefined) {
            console.log('BPlayerApp - it looks like popup window is blocked. > show alert');


            $.featherlight('<h1>Unable to open popup window</h1><p>Please make sure to allow popups for sonicsquirrel.net</p>', {variant: 'warning'});

        } else {
            if (typeof (win.bplayer_loaded) == 'undefined') {
        ...
        var unloadInterval = setInterval(function () {
            if (!self.remote_window || self.remote_window.closed) {
    ...
    this.remote_unload = function() {
        console.log('remote window gone! > unload player');
        self.remote = false;
    };
    ...
    this.bindings = function () {
        ...
        $('body').on('click', 'a[data-bplayer-action]', function (e) {
            ...
            $.get(uri, function (data) {
                ...
                // delegate to delayed control
                self.delay_controls(action, playlist)
                ...
                /*
                 ...
                 alert('not ready')
                 setTimeout(function () {
                 self.remote.set_playlist(playlist);
                 self.remote.controls({action: 'play', index: 0});
                 }, 5000)
                 } else {
                 self.remote.set_playlist(playlist);
                 self.remote.controls({action: 'play', index: 0});
        ...
        $('body').on('click', '#bplayer_playlist_container .item', function (e) {
            ...
            if(!$(e.target).is('a')) {
                ...
                try {
                    self.remote.controls({action: 'play', index: index});
                } catch(e) {
                    console.log(e);
                }
            }
        ...
        $('body').on('click', 'a[data-bplayer-controls]', function (e) {
            ...
            try {
                self.remote.controls({action: action});
            } catch(e) {
                console.log(e);
            }
    ...
    this.delay_controls = function (action, playlist) {
        // not so nice implementation... TODO: think about a nicer way

        //console.log('BPlayerApp - delay_controls', action, playlist);

        if (!self.remote) {

            if (self.remote_try_count == 10000) {
                // console.log('BPlayerApp - it looks like popup window is blocked. > show alert');
            }

            self.remote_try_count++;
            //console.log('BPlayerApp - remote player not ready/available yet. tried ' + self.remote_try_count + ' times.');
            setTimeout(function () {
                self.delay_controls(action, playlist);
            }, 10)
        } else {
            console.log('BPlayerApp - remote player seems to be ready -> sending controls:', action, playlist);
            if (action == 'play') {
                self.remote.set_playlist(playlist);
                self.remote.controls({action: 'play', index: 0});
            }
            if (action == 'queue') {
                self.remote.set_playlist(playlist, true);
    ...
    this.update_player = function () {
        ...
        console.log('remote player told me to update!');

        var media = self.remote.playlist[self.remote.current_index];
        ...
        if (media != undefined) {
            ...
            $('#bplayer_playlist_container', self.container).html(nj.render('bplayer/nj/playlist.html', {
                objects: self.remote.playlist
            }));


            // update summary
            var totals = {
                index: self.remote.current_index + 1,
                num_tracks: self.remote.playlist.length,
                duration: self.remote.playlist.length * 60
            }
        ...
        // hack: check remote state, as else handled by events which could already be fired earlier

        if(bplayer.remote.state == 'playing') {
            self.state_change('playing');
        }
