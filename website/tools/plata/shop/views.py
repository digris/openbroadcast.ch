...
class Shop(object):
    ...
    def cart(self, request, order):
        ...
        if request.method == 'POST':
            ...
            discount_form = DiscountForm(request.POST, **kwargs)


            # catch the discount-form
            if 'save_discount' in request.POST:
            ...
            discount_form = DiscountForm(**kwargs)
    ...
    def discounts(self, request, order):
        ...
        if not self.include_discount_step(request) or True:
            return self.redirect('plata_shop_confirmation')
    ...
    def order_success(self, request):
        ...
        return self.render(
            ...
            self.get_context(
                request, {
                    'order': order,
                    'order_fix': order, # TODO: investigate!! order somehow gets blanked in template???
                    'progress': 'success',
                }
            )
        )
